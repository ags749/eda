...
_installCommonDev() {
    ...
    if [[ ! -f ${cmakeBin} || -z $(${cmakeBin} --version | grep ${cmakeVersionBig}) ]]; then
        cd "${baseDir}"
        #cp /home/ubuntu24/cmake-3.24.2-linux-x86_64.sh .
    ...
    if [[ -z $(grep "BOOST_LIB_VERSION \"${boostVersionBig//./_}\"" ${boostPrefix}/include/boost/version.hpp) ]]; then
        ...
        #cp /home/ubuntu24/boost_1_80_0.tar.gz .
    ...
    if [[ ! -f ${cuddPrefix}/include/cudd.h ]]; then
        cd "${baseDir}"
        git clone --depth=1 -b ${cuddVersion} https://gitee.com/nihaoa123fylfyl/cudd.git
        cd cudd
        autoreconf
    ...
    if [[ ! -d ${cuspPrefix}/cusp/ ]]; then
        cd "${baseDir}"
        git clone --depth=1 -b cuda9 https://gitee.com/xlindo/cusplibrary.git
        cd cusplibrary
    ...
    if [[ -z $(grep "LEMON_VERSION \"${lemonVersion}\"" ${lemonPrefix}/include/lemon/config.h) ]]; then
        cd "${baseDir}"
        git clone --depth=1 -b ${lemonVersion} https://gitee.com/zrshh/lemon-graph.git
        cd lemon-graph
    ...
    if [[ ! -d ${spdlogPrefix}/include/spdlog ]]; then
        cd "${baseDir}"
        git clone --depth=1 -b "v${spdlogVersion}" https://gitee.com/mirror-luyi/spdlog.git
        cd spdlog
...
_installOrTools() {
    ...
    if [ "$(uname -m)" == "aarch64" ]; then
        ...
        git clone --depth=1 -b "v${orToolsVersionBig}" https://gitee.com/benkangpeng/or-tools.git
        cd or-tools
        ...
        #cp /home/ubuntu24/Downloads/or-tools_amd64_ubuntu-20.04_cpp_v9.11.4210.tar.gz .
